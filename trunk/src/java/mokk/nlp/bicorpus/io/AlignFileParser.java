 ...
 */
public class AlignFileParser implements 
FileDocumentSource, Component, LogEnabled, Configurable {
     ...
     public void read(File inputFile) throws ProcessingException, IOException {
	   ...
	   if(inputFile.getName().endsWith(".gz")) {
		...
		while((line = reader.readLine()) != null ){
		    ...
		    int i = line.indexOf("\t");
		    if(i < 0) {
		        throw new ProcessingException("no tab found in the line:" + reader.getLineNumber());
		    }
		
		    String left = line.substring(0, i);
		    String right = line.substring(i+1);

		    BiSentence bis = new BiSentence(source, "" + reader.getLineNumber()  , left, right);
