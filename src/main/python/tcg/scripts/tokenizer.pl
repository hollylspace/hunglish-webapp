...
sub tokenize {
	...
	if ($language eq "en") {
		...
		# bpgergo special hunglish treatment
		my @keys = qw(do is does are has had was were have might need did would should could must);
		for my $keyindex (0..$#keys) {
			$text =~ s/(\s)$keys[$keyindex]n 't(\s)/$1$keys[$keyindex] n't$2/ig;
		}
		# bpgergo hunglish exceptions 
		$text =~ s/(\s)won 't(\s)/$1will n't$2/ig;
		$text =~ s/(\s)can 't(\s)/$1can n't$2/ig;
		$text =~ s/(\s)ain 't(\s)/$1ai n't$2/ig; #!
