...
def main() :
	if len(sys.argv) == 4:
		...
		def parseLadderLine(l) :
		    a = l.split()
		    assert len(a)==3
		    return ( int(a[0]), int(a[1]), a[2] ) # The score we leave as a string,
		    # to avoid small diffs caused by different numerical representations.
		ladder = map( parseLadderLine, ladderlines )

		if len(ladder)<=1 :
		    # This is an ugly special case that should never occur,
		    # as even in the worst case we know 0<->0 and len(hulines)<->len(enlines).
		    # But it does actually occur, because hunalign returns an empty file
		    # if it does not like the input.
		    ladder = []
		    ladder.append( ( 0, 0, "0" ) )
		    ladder.append( ( len(hulines), len(enlines), "0" ) )

		# the next map() does all the work, so here are some comments...
		# the map() iterates over the holes of the ladder. 
		# a hole is supposed to be two consecutive items in the array holding the lines of the ladder.
		...
		print 'usage: ladder2text.py <hu.raw> <en.raw> <aligned.ladder> > aligned.txt'
		sys.exit(-1)
